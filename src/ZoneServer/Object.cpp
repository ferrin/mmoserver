...
void Object::sendAttributes(PlayerObject* playerObject)
{
	...
	while(orderIt != mAttributeOrderList.end())
	{
		...
		if(gWorldManager->getAttributeKey((*mapIt).first).getCrc() == BString("duration").getCrc())
		{
			...
			int8 valueInt[64];
			snprintf(valueInt, sizeof(valueInt),"%um %us",minutes,seconds);
			value = valueInt;
...
//	int8 sql[1024]
	snprintf(sql,sizeof(sql),"UPDATE item_attributes SET value='");

	sqlPointer = sql + strlen(sql);
	sqlPointer += gWorldManager->getDatabase()->Escape_String(sqlPointer,value.c_str(),value.length());
	snprintf(restStr, sizeof(restStr),"'WHERE item_id=%I64u AND attribute_id=%u",this->getId(),attributeID);
	strcat(sql,restStr);

	
	//snprintf(sql,sizeof(sql),"UPDATE item_attributes SET value='%s' WHERE item_id=%"PRIu64" AND attribute_id=%u",value,this->getId(),attributeID);
...
void Object::addAttributeIncDB(string key,std::string value)
{
	...
	int8 sql[512],*sqlPointer,restStr[128];
//	int8 sql[1024]
	snprintf(sql,sizeof(sql),"INSERT INTO item_attributes VALUES(%"PRIu64",%u,'",this->getId(),attributeID);

	sqlPointer = sql + strlen(sql);
	sqlPointer += gWorldManager->getDatabase()->Escape_String(sqlPointer,value.c_str(),value.length());
	snprintf(restStr, sizeof(restStr),"',%u,0)",static_cast<uint32>(this->getAttributeMap()->size()));
	strcat(sql,restStr);
	
	gWorldManager->getDatabase()->ExecuteSqlAsync(0,0,sql);

	//snprintf(sql,sizeof(sql),"INSERT INTO item_attributes VALUES(%"PRIu64",%u,%s,%u,0)",this->getId(),attributeID,value,mAttributeOrderList.size());
}
...
//	int8 sql[1024]
	...
	sqlPointer = sql + strlen(sql);
	sqlPointer += gWorldManager->getDatabase()->Escape_String(sqlPointer,value.c_str(),value.length());
	snprintf(restStr, sizeof(restStr),"',%u,0)",static_cast<uint32>(this->mInternalAttributeMap.size()));
	strcat(sql,restStr);
