...
void WorldConfig::handleDatabaseJobComplete(void* ref,DatabaseResult* result)
{
	...
	if(!mLoadComplete)
	{
		mLoadComplete = true;
		int8 sql[255];
		snprintf(sql,sizeof(sql),"SELECT csa.attribute,cs.value FROM config_server cs INNER JOIN config_server_attributes csa ON (csa.id = cs.config_attributes_id) WHERE cs.server_name like '%s' ",mZoneName.getAnsi());
