...
//	int8 address[32] = boost::asio::ip::address::to_string()(endpoint.address();//(endpoint.address);
	...
	int8 sql[500];
	snprintf(sql,sizeof(sql),"SELECT id, address, port, status, active FROM config_process_list WHERE address='%s' AND port=%u;", session->getAddressString(), session->getPortHost());
